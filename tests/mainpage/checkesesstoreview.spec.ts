...
test('test', async ({ page }) => {
  ...
  await mainPage.switchtoesesstoreview();
  ...
  for (const h1Element of h1Elements) {
    ...
    if (text && text.trim() === 'Peri\u00F3dicos & Revistas hist\u00F3ricos como idea para regalo') { // Очікуваний заголовок
      isH1Correct = true;
      break;
    }
  }
  ...
  try {
    await page.waitForURL('https://www.historia.net/es-es/'); // Перевіряємо URL
