...
test('test', async ({ page }) => {
  ...
  await mainPage.switchtonlnlstoreview();
  ...
  for (const h1Element of h1Elements) {
    ...
    if (text && text.trim() === 'Oude kranten en tijdschiften van de geboortedag als cadeau') { // Очікуваний заголовок
      isH1Correct = true;
      break;
    }
  }
  ...
  try {
    await page.waitForURL('https://www.historia.net/nl-nl/'); // Перевіряємо URL
