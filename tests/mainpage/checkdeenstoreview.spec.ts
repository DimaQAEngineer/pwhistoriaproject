...
test('test', async ({ page }) => {
  ...
  await mainPage.switchtodeenstoreview();
  ...
  for (const h1Element of h1Elements) {
    ...
    if (text && text.trim() === 'Historical newspapers & magazines as a gift idea') { // Очікуваний заголовок
      isH1Correct = true;
      break;
    }
  }
  ...
  try {
    await page.waitForURL('https://www.historia.net/de-en/'); // Перевіряємо URL
