...
test('test', async ({ page }) => {
  ...
  await mainPage.switchtoplplstoreview();
  ...
  for (const h1Element of h1Elements) {
    ...
    if (text && text.trim() === 'Historyczne gazety i czasopisma jako pomysł na prezent') { // Очікуваний заголовок
      isH1Correct = true;
      break;
    }
  }
  ...
  try {
    await page.waitForURL('https://www.historia.net/pl-pl/'); // Перевіряємо URL
